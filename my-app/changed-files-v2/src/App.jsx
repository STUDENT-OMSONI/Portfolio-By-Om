...
function useParallax(disabled) {
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (nameRef.current)  nameRef.current.style.transform  = `translate(${nx * -14}px, ${ny * -8}px)`;
      if (photoRef.current) photoRef.current.style.transform = `translateX(-50%) translate(${nx * 10}px, ${ny * 6}px)`;
    };
...
function Logo({ size = 24 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <rect x="1" y="1" width="38" height="38" rx="9" stroke="currentColor" strokeWidth="1.4" opacity="0.9" />
      <path d="M12 27V13.5L20 22L28 13.5V27" stroke="currentColor" strokeWidth="2.1" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
const PROJECTS = [
  ...
  {
    id: 3,
    title: 'Robot Motion Data Pipeline',
    cat: 'Trident Groups — Field Data Collection',
    stack: ['Python', 'OpenCV', 'Data Annotation', 'Data Pipelines'],
    img: project3Img,
    detail: 'Operated the Magic Hat device (camera + voice sensor) to capture real-time factory-floor worker motion, then structured and labelled the resulting motion-capture datasets to train robotic systems — bridging raw field recordings into clean, annotated data for machine-learning pipelines used in industrial automation.',
  },
];
  ...
  return (
    <div className={`intro-screen ${exiting ? 'exiting' : ''}`}>
      <div className="intro-icons">
        <div className="intro-icon-btn" title="Analyst">📊</div>
        <div className="intro-icon-btn" title="Engineer">👤</div>
        <div className="intro-icon-btn" title="AI">🤖</div>
      </div>

      <h1 className="intro-heading">
        Welcome to my<br />Portfolio Website
      </h1>
      <p className="intro-sub">Turning data into decisions, one dataset at a time.</p>
  ...
  return (
    <>
      ...
      <nav className={`nav ${navScrolled ? 'scrolled' : ''} nav-slide-in`}>
        <div className="container nav-inner">
          <div className="nav-brand">
            <div className="nav-brand-icon"><Logo size={18} /></div>
            OM SONI · DATA ANALYTICS
          </div>
      ...
      <section id="home" className="hero">
        ...
        <div className="hero-name-overlay" aria-hidden="true" ref={heroNameRef}>OM</div>

        {/* Photo */}
        <div className="hero-photo-wrap" ref={heroPhotoRef}>
          <img src={omPhoto} alt="Om Soni" />
        </div>

        {/* Right text */}
        <div className="hero-right-text">
          <span className="hero-right-line">Data Analytics</span>
          <span className="hero-right-line">&amp; AI /</span>
          <span className="hero-right-line">Data Science.</span>
        </div>

        {/* Short supporting line */}
        <p className="hero-subline">
          AI &amp; Data Science student building data-driven, intelligent digital experiences.
        </p>
        ...
        <div className="hero-bottom">
          ...
          <div className="hero-btn-row">
            <button ref={viewWorkBtnRef} className="hero-pill-btn" data-cursor="VIEW" onClick={() => scrollTo('work')}>
              VIEW MY WORK ↗
            </button>
            <a ref={resumeBtnRef} href="/Om_Soni_Resume.pdf" download className="hero-pill-btn hero-pill-btn-ghost">
              DOWNLOAD RESUME
            </a>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="sc-bottom">
                    <div>
                      ...
                      <div style={{ fontSize: '0.82rem', fontFamily: 'var(--font-ui)', color: 'var(--ink)', fontWeight: 600, marginTop: 4 }}>
                        {p.title}
                      </div>
                    </div>
